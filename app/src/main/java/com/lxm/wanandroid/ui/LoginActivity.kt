...
class LoginActivity : AppCompatActivity() {
    ...
    private fun initView() {
        ...
        (login.registerView as DefaultRegisterView).setListener { registerUser, registerPass, registerPassRep ->
            if (!TextUtils.isEmpty(registerUser.editText?.text) && !TextUtils.isEmpty(registerPass.editText?.text) && !TextUtils.isEmpty(registerPassRep.editText?.text)) {
                model.register(registerUser.editText?.text.toString(),registerPass.editText?.text.toString(),registerPassRep.editText?.text.toString())
            } else {
                ToastUtils.showToast("用户名和密码不能为空", Toast.LENGTH_SHORT)
            }
        }
